...
function translateKnownProductText(value: any, language: 'en' | 'ar', t: (key: string) => string) {
  if (typeof value !== 'string') return localized(value, language);
  if (value.trim() === 'A refined essential with a considered fit and effortless finish.') return t('product.descriptionFallback');
  if (value.trim() === 'A refined essential with a considered fit and effortless finish') return t('product.descriptionFallback');
  return value;
}
...
export default function ProductDetailsSection({ product, variant }: { product: any; variant: any }) {
  ...
  return <section className="lg:sticky lg:top-28 self-start">
    <div className="flex items-center justify-between gap-4"><p className="text-[10px] uppercase tracking-[.2em] text-muted">fabora / {product.status ? t(`product.status.${String(product.status).toLowerCase()}`, { defaultValue: String(product.status) }) : ''}</p><span className="text-[10px] uppercase tracking-[.16em] text-muted">{product.categoryHierarchy?.[0] ? t(`nav.${String(product.categoryHierarchy[0]).toLowerCase()}`, { defaultValue: String(product.categoryHierarchy[0]) }) : t('product.collection')}</span></div>
    <div className="mt-4 pb-6 border-b thin-border">
      ...
      <p className="text-sm leading-7 text-muted mt-5 max-w-xl">{translateKnownProductText(product.description, language, t) || t('product.descriptionFallback')}</p>
    </div>
    ...
    <div className="border-t thin-border"><Accordion title={t('product.details')}><p>{translateKnownProductText(product.description, language, t) || t('product.descriptionFallback')}</p></Accordion><Accordion title={t('product.shipping')}><p>{t('product.shippingText')}</p></Accordion><Accordion title={t('product.care')}><p>{t('product.careText')}</p></Accordion></div>
  </section>;
}
