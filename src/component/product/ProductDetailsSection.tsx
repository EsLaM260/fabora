...
export default function ProductDetailsSection({ product, variant }: { product: any; variant: any }) {
  ...
  const [size, setSize] = useState(getAttr(variant, 'size') || sizes[0] || 'M');
  const [qty, setQty] = useState(1);

  useEffect(() => {
    const colorVariants = variants.filter((v: any) => String(getAttr(v, 'color')).toLowerCase() === String(selectedColor).toLowerCase());
    const colorSizes = colorVariants.map((v: any) => getAttr(v, 'size')).filter(Boolean).map(String);
    if (colorSizes.length && !colorSizes.some((value) => value.toLowerCase() === String(size).toLowerCase())) {
      setSize(colorSizes[0]);
    }
  }, [selectedColor, size, variants]);
  const selectedVariant = useMemo(() => {
    const matches = variants.filter((v: any) => String(getAttr(v, 'color')).toLowerCase() === String(selectedColor).toLowerCase());
    return matches.find((v: any) => String(getAttr(v, 'size')).toLowerCase() === String(size).toLowerCase()) || matches[0] || variant;
  }, [selectedColor, size, variant, variants]);
